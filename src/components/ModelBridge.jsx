...
export default function ModelBridge() {
  ...
  return <main className={`app-shell ${state.mode === 'macro' ? 'macro-layout' : ''}`}>
    ...
    {panel === 'directory' && <AgentDirectory agents={state.agents} onClose={() => { closePanels(); noteTutorialAction('directory-close') }} onFind={findAgent} onFollow={followAgent} onHighlight={id => { setHighlightedBuilding(null); setHighlightedAgent(current => current === id ? null : id) }} onDetails={id => { noteTutorialAction('directory-select'); selectAgent(id); setPanel('directory') }} />}
