  ...
  return (
    <section className={`absolute inset-0 z-50 overflow-y-auto bg-[#05070df2] px-4 py-16 transition-opacity duration-700 lg:px-10 ${visible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <div className="mx-auto max-w-6xl">
        <p className="text-xs uppercase tracking-[.35em] text-cyan-300">Scene 07 / Policy Lab</p>
        <div className="mt-3 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-4xl font-light md:text-6xl">Change the system.</h2><p className="mt-2 max-w-xl text-sm text-slate-400">Explore how policy levers move outcomes across agents, networks, and institutions.</p></div><span className="rounded-full border border-emerald-300/20 bg-emerald-300/8 px-4 py-2 text-xs text-emerald-300">Live illustrative model</span></div>
        <div className="mt-8 grid gap-5 lg:grid-cols-[.78fr_1.4fr]">
          <div className="glass rounded-3xl p-5"><h3 className="mb-5 text-sm uppercase tracking-[.22em] text-slate-300">Policy levers</h3>{sliders.map(([key, label]) => <label className="mb-5 block" key={key}><span className="mb-2 flex justify-between text-xs text-slate-300"><span>{label}</span><strong className="text-cyan-300">{policies[key]}</strong></span><input className="range" type="range" min="0" max="100" value={policies[key]} onChange={e => setPolicies({ ...policies, [key]: Number(e.target.value) })} /></label>)}</div>
