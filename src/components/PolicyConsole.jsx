...
export default function PolicyConsole({ state, onApply, onSpeed }) {
  ...
  const chartMax = Math.max(1, ...history.map(point => point.consumption), Number(outcomes.consumption))
  const chartPoints = history.map((point, index) => `${history.length <= 1 ? 0 : index / (history.length - 1) * 100},${48 - point.consumption / chartMax * 44}`).join(' ')
  return <section className="macro-console">
    ...
    <div className="panel history-chart"><div><p className="kicker">Output history · rolling 24 hours</p><strong>Alcohol consumption: {outcomes.consumption} units / resident</strong></div><svg viewBox="0 0 100 50" preserveAspectRatio="none"><path d="M0 48H100" /><polyline points={chartPoints || '0,48 100,48'} /></svg></div>
