...
export default function DetailsPanel({ building, agent, agents = [], buildings = [], mode = 'micro', occupants = [], residents = [], onClose, onSelectAgent, onSelectBuilding, onFollowAgent, onMove, anchor = { x: 50, y: 50 } }) {
  ...
  const panelEdge = position.x > anchor.x ? { x: position.x - 17, y: position.y + 5 } : { x: position.x + 17, y: position.y + 5 }
  const bendY = anchor.y < panelEdge.y ? anchor.y - 4 : anchor.y + 4
  const path = `M ${anchor.x} ${anchor.y} L ${anchor.x} ${bendY} Q ${anchor.x} ${panelEdge.y} ${anchor.x + (panelEdge.x > anchor.x ? 2 : -2)} ${panelEdge.y} L ${panelEdge.x} ${panelEdge.y}`
  ...
  return <>
    {!mobile && <svg className="detail-connector" viewBox="0 0 100 100" preserveAspectRatio="none"><path d={path} /></svg>}
    <aside className={`panel details-panel sheet-${sheet}`} style={mobile ? undefined : { left: `${position.x}%`, top: `${position.y}%` }}>
      <div className="section-heading drag-handle" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={() => { drag.current = null }}>
      ...
      {agent && mode === 'meso' ? <>
        <p className="kicker list-heading">Social network</p><div className="occupant-list">{friends.map(friend => <button key={friend.id} onClick={() => onSelectAgent(friend.id)}><i style={{ background: friend.color }} />{friend.name}<span>{friend.insideBuildingId ? 'At a place' : 'Travelling'}</span></button>)}</div>
        <p className="kicker list-heading">Domicile</p><div className="relation-list">{relationButton(domicile, 'Home and recovery')}</div>
        <p className="kicker list-heading">Work</p><div className="relation-list">{relationButton(workplace, 'Workplace')}</div>
        <p className="kicker list-heading">Favorite gathering place</p><div className="relation-list">{relationButton(gatheringPlace, 'Social gathering place')}</div>
      </> : agent ? <>
