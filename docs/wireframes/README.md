...
Updated through `v0.3.0-beta.8`: responsive desktop/laptop foundations and the implemented Agent Directory card hierarchy, browsing controls, and Details/Locate/Follow actions.
...
Tutorial-specific interaction rules are documented in `../TUTORIAL_IMPLEMENTATION_BETA8.md`. Buildings are selectable throughout the tutorial, speed controls are restricted to the requested tutorial action, and restoration resets the simulation and graph-history boundary trackers to the saved snapshot.

## Beta.8 Wireframe Inventory

- Desktop Micro: brand/settings header, stacked date and playback controls, Agent Directory card hierarchy and browsing controls, billboard-circle occupancy towers, camera presets, tutorial step 12 status, mode navigation, and Help.
